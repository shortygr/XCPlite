...
float mes_unit8 = 0.0f;
float mes_unit16 = 0.0f;

void signaltask(float& signal)
{
    for (;;)
    {
        signal = mes_unit16 * (double)clockGet64() * M_2PI;
...
int main(int argc, char* argv[]) {
    ...
    a2l->createMeasurement(mes_unit8, "Signal gain uint8");
    a2l->createMeasurement(mes_unit16, "Signal gain uint16");
    ...
    std::thread st(signaltask, std::ref(mes_unit16));
